...
const createAuthor = async function (req, res) {
  try {
    ...
    if (!authorData.lname)
      return res.status(400).send({ status: false, msg: "Please include the last name" });
    ...
    if (!authorData.email)
      return res.status(400).send({ status: false, msg: "Please include an email" });
    ...
    if (!authorData.password)
      return res.status(400).send({ status: false, msg: "Please include a password" });
